...
def train(
    ...
    for epoch in range(args.start_epoch, training_box.num_epochs):
        ...
        if tracker is not None:
            tracker.log({'val/acc1': val_top1_accuracy, 'epoch': epoch}, step=(epoch + 1) * len(training_box.train_data_loader))
...
def main(args):
    ...
    if tracker is not None:
        tracker.log({'test/acc1': test_top1_accuracy})
        tracker.finish()
