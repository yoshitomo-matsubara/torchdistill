...
def get_argparser():
    parser = argparse.ArgumentParser(description='Knowledge distillation for information retrieval models')
    parser.add_argument('--config', required=True, help='yaml file path')
    parser.add_argument('--run_log', help='log file path')
    parser.add_argument('--task_name', type=str, default=None, help='name of the task to train on.')
...
def evaluate(model, data_loader, metric, accelerator, title=None, header='Test: '):
...
def train(teacher_model, student_model, dataset_dict, dst_ckpt_dir_path, metric,
    ...
    for epoch in range(training_box.num_epochs):
        ...
        val_dict = evaluate(student_model, training_box.val_data_loader, metric, accelerator, header='Validation: ')
...
def main(args):
    ...
    dataset_dict, label_names_dict = \
        get_all_datasets(config['datasets'], task_name, student_tokenizer, student_model) # TODO: replace with one extracted from config

    # Update config with dataset size len(data_loader)
    customize_lr_config(config, dataset_dict, world_size) # TODO: replace with one extracted from config
    ...
    metric = get_metrics(task_name) # TODO: replace with one extracted from config

    if not args.test_only:
        train(teacher_model, student_model, dataset_dict, dst_ckpt_dir_path, metric,
              device, device_ids, distributed, config, args, accelerator)
    ...
    if not args.student_only and teacher_model is not None:
        ...
        evaluate(teacher_model, test_data_loader, metric, accelerator,
    ...
    evaluate(student_model, test_data_loader, metric, accelerator,
